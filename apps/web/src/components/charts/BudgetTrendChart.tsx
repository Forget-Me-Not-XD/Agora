    ...
    return (
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl px-3 py-2 shadow-lg text-xs">
            ...
            <p className="text-[var(--color-primary)] font-semibold">
                Begroting: {fmtRand(Number(payload[0].value))}
            </p>
        </div>
    );
}

export default function BudgetTrendChart({ data }: { data: SingleMonthlyPoint[] }) {
        ...
        <div>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-3">
                <span className="flex items-center gap-1.5 text-xs text-[var(--color-text-subtle)]">
                    ...
                    Begroting <span className="font-semibold text-[var(--color-text)]">{fmtRand(total)}</span>
                </span>
            ...
            <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={data} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="value" name="Begroting" stroke="var(--color-primary)" fill="url(#fillBudget)" strokeWidth={2} />
                </AreaChart>
            </ResponsiveContainer>
        </div>
    );
